...
export class GameManager extends Component implements APIHandler {
  ...
  body_lean: BodyLean = BodyLean.NONE
  ...
  onClickStartGame() {
    ...
    this.scheduleOnce(this.leanCamera, 10)
  }

  rotateCamera(angle: number) {
    tween(this.camera3d.node).to(0.2, { eulerAngles: new Vec3(-7, -90, angle) }).start()
  }
  ...
  resetCamera() {
    this.unschedule(this.leanCamera)
    this.rotateCamera(0)
    this.body_lean = BodyLean.NONE
  }
  ...
  finishLevel() //called by BigTarget
  {
    ...
    if (!!LevelData.user_data)
      this.api.updateResult(
        LevelData.user_data._id,
        LevelData.levels[LevelData.current_level]._id,
        this.score,
        percent
      )
  ...
  onDetectPose(response: DetectPoseResponse) {
    ...
    if (guide == MoveGuide.OK && this.gamestate == GameState.PLAY) {
      this.processNewPoint(true, response.keypoints[10])
      this.processNewPoint(false, response.keypoints[9])
      this.calculateBodyAngle(response.keypoints[6], response.keypoints[7], response.keypoints[12], response.keypoints[13])
    }
